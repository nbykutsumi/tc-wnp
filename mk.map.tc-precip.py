...
for scen in lscen:
    ...
    for iens, ens in enumerate(lens):
        precbasedir = "/home/utsumi/temp/bams2020/tc-prec-all"
        a3sum[iens] = np.array([np.load(precbasedir + "/%s.%03d/sum.%04d.npy"%(scen,ens,Year)) for Year in lYear]).sum(axis=0)
        a3num[iens] = np.array([np.load(precbasedir + "/%s.%03d/num.%04d.npy"%(scen,ens,Year)) for Year in lYear]).sum(axis=0)

    a2sum = a3sum.sum(axis=0)
    a2num = a3num.sum(axis=0)

    a2fig = (ma.masked_where(a2num==0, a2sum) / a2num)*60*60*24
