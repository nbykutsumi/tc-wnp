...
for norm in ["org"]:
    ...
    if norm=="org":
        fig, axs = plt.subplots(nrows=nrow, ncols=ncol, figsize=(120,120))
    elif norm=="norm":
    ...
    for isub,subregion in enumerate(lsubregion):
        ...
        for scen in lscen:
            ...
            if scen=="HPB_NAT":
                ngrids = (~d2mask[subregion]).sum()
                ndays = (datetime(eY,12,31)-datetime(iY,1,1)).days 
                ntot = len(lens)* ndays * ngrids
                a1ptile = 1 - np.cumsum(a1freq[::-1])[::-1] / ntot
                print(a1ptile)
        ...
        wbar = bnd[1] - bnd[0]
        ax.bar(bndcnt, a1his, width=wbar, alpha=0.5, tick_label=a1ptile, color="red", log=True)
        ax.bar(bndcnt, a1nat, width=wbar, alpha=0.5, tick_label=a1ptile, color="blue", log=True)
        ax.set_xticks(bndcnt)
        ax.set_xticklabels(a1ptile, rotation=70)
        #ax.set_xlabel(a1ptile)
    ...
    #figpath= figdir + "/temp.hist.mul.tc-prec.%s.png"%(norm)
    #plt.savefig(figpath)

    figpath= figdir + "/temp.hist.mul.tc-prec.%s.pdf"%(norm)
    plt.savefig(figpath)
