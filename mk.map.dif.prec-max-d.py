...
for scen in ["HPB","HPB_NAT"]:
    ...
    for iens,ens in enumerate(lens):
        print(scen,ens)
        datbasedir = "/home/utsumi/temp/bams2020/max-prec-d"
        a3dat[iens] = np.array([np.load(datbasedir + "/%s.%03d/ann-max-prec.%04d.npy"%(scen,ens,Year)) for Year in lYear]).mean(axis=0)
...
a2dif = (a3dat_his.mean(axis=0) - a3dat_nat.mean(axis=0))*60*60*24   # mm/day
...
#-- title, figure name -----
stitle = 'diff. annual max (HIST - NAT) mm/day \n' + '%04d-%04d ens:%03d-%03d'%(iY,eY,lens[0],lens[-1]) 
figpath = figdir + '/map.dif.prec-ann-max.sim.%04d-%04d.png'%(iY,eY)
#---------------------------
...
cmbnd = list(range(-25,25+1,5))
cmlabels = list(range(-25,25+1,5))
mycm = 'RdBu_r'
