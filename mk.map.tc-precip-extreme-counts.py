...
#-- Load annual max (NAT) ---
maxdir = "/home/utsumi/temp/bams2020/max-prec-d/HPB_NAT.med"
maxpath= maxdir + "/ann-max-prec.ens-001-050.1990-2010.npy"
a2max = np.load(maxpath)    # original unit
        ...
        for Year in lYear:
            ...
            for Mon in lMon:
                ...
                for i,DTime in enumerate(lDTime):
                    ...
                    a2mask = ma.masked_where(a3prec_day[i] <a2max, a2mask)
...
for scen in lscen:
    ...
    a2num = a3num.mean(axis=0) /len(lYear)*10  # times/10-year
    ...
    cmbnd = list(np.arange(0,10+0.1,0.5))
    cmlabels = list(np.arange(0,10+1,2))
    ...
    #-- title, figure name -----
    figpath = figdir + '/map.prec-tc.count-extreme.%s.%04d-%04d.png'%(scen,iY,eY)
    stitle = 'Count TC extreme prec. times/10-yr %s\n'%(scen) + '%04d-%04d ens:%03d-%03d'%(iY,eY,lens[0],lens[-1])
